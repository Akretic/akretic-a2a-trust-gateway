...
def _platform_badge() -> str:
    if _is_cloud_mode():
        return '<span class="label info">Cloud Run + Vertex Gemini proof</span>'
...
def _model_summary_label(model_summary: dict[str, Any]) -> str:
    if model_summary.get("mode") == "vertex":
        return "Generated by Vertex Gemini from permitted source IDs only."
...
def _model_story_text(model_summary: dict[str, Any]) -> str:
    if model_summary.get("mode") == "vertex":
        return "Vertex Gemini summarizes permitted sources only."
...
def _default_status() -> str:
    if _is_cloud_mode():
        return "Cloud Run + Vertex Gemini proof"
...
def _home_proof_story() -> str:
    ...
    <section class="proof-story-block">
      ...
      <p class="panel-note">
        ...
        seeded public research, Gemini summarization, A2A calls, approval, and evidence.
      </p>
      <div class="proof-story" aria-label="Eight step proof path">
        ...
        <article class="story-step info">
          ...
          <h3>Gemini</h3>
          <div>Model summarization uses permitted sources only; cloud mode uses Vertex Gemini.</div>
        </article>
        <article class="story-step ok">
            ...
            rows.append(
                {
                    ...
                    "hashes": f"req {request_hash[:16]} / resp {response_hash[:16]}",
    ...
    *,
    ...
    <section class="panel story-panel">
      ...
      <div class="proof-story" aria-label="Run evidence proof path">
        ...
        {_story_step(4, "Gemini", f'{html.escape(model_story)} Mode <span class="code-chip">{html.escape(mode)}</span>; model <span class="code-chip">{html.escape(model)}</span>; project <span class="code-chip">{html.escape(project_id)}</span>; location <span class="code-chip">{html.escape(location)}</span>.', "info")}
        ...
        {_story_step(7, "Evidence", f'<span class="code-chip">{html.escape(valid_label)}</span>; event count <span class="code-chip">{html.escape(event_count)}</span>; <a href="{evidence_href}" target="_blank" rel="noreferrer">current-run evidence</a>.', evidence_state)}
      </div>
    </section>
    """
